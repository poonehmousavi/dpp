...
class SALMONN(nn.Module):
    ...
    def from_config(cls, config):
        ...
        prompt_strategy = config.get("prompt_strategy", "sim_strategy")
        similarity_method = config.get("similarity_method", "cosine")
        residual_threshold = config.get("residual_threshold", 1e-3)
        attn_threshold = config.get("attn_threshold", 0.05)
        use_prompt_threshhold= config.get("use_prompt_threshhold",False)
